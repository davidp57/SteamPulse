...
6. [Prérequis GOG Galaxy](#6-prérequis-gog-galaxy)
7. [Xbox PC Game Pass](#7-xbox-pc-game-pass)
8. [Tout-en-un — `steampulse.exe`](#8-tout-en-un--steampulseexe)
9. [Options complètes](#9-options-complètes)
10. [Naviguer dans l'interface](#10-naviguer-dans-linterface)
11. [Stratégie de cache et rafraîchissement](#11-stratégie-de-cache-et-rafraîchissement)
12. [Usage avancé — étapes séparées](#12-usage-avancé--étapes-séparées)
13. [Configuration web (Docker / sidecar)](#13-configuration-web-docker--sidecar)
14. [FAQ](#14-faq)
15. [Déploiement Docker](#15-déploiement-docker)
16. [Mode live — steam-serve](#16-mode-live--steam-serve)
...
3. **GOG Galaxy** (optionnel) — flux OAuth2 similaire ; ouvre l'URL de login GOG, demande le code depuis l'URL de redirection, et sauvegarde un refresh token.
4. **Xbox PC Game Pass** (optionnel) — simple oui/non ; aucune authentification requise (catalogue public).
5. **Twitch/IGDB** (optionnel) — client ID et secret pour une meilleure résolution Epic→Steam AppID
6. **Paramètres** (optionnel) — chemin de la base, threads de travail, âge des news, langue
...
## 6. Prérequis GOG Galaxy

L'intégration GOG est **optionnelle**. Passe cette section si tu n'utilises pas GOG.

### Authentification — assistant de configuration

Lance `steam-setup` et suis l'étape 3 (GOG Galaxy). L'assistant ouvre l'URL de login GOG dans ton navigateur, te demande le code d'autorisation depuis l'URL de redirection, et sauvegarde un refresh token dans `config.toml`. Les lancements suivants réutilisent le refresh token automatiquement.

L'entrée de config écrite par l'assistant :
...
### Authentification — ligne de commande
...
L'intégration Game Pass est **optionnelle** et ne nécessite **aucune authentification** — elle lit le catalogue public Microsoft.

Active-la dans l'assistant (étape 4) ou via la config :
...
Les jeux Game Pass sont tagués `gamepass` dans le dashboard et enrichis avec les détails Steam si un AppID correspondant est trouvé.

---

## 8. Tout-en-un — `steampulse.exe`
...
| Option | Défaut | Description |
|---|---|---|
| `--gog-refresh-token` | *(aucun)* | Refresh token GOG (sauvé automatiquement après l'assistant) |

**Option Xbox Game Pass :**

| Option | Défaut | Description |
|---|---|---|
| `--game-pass` | désactivé | Activer la découverte de la bibliothèque Xbox PC Game Pass |
...
## 11. Stratégie de cache et rafraîchissement
...
## 12. Usage avancé — étapes séparées
...
## 13. Configuration web (Docker / sidecar)

Lorsque `steam-serve` tourne (ou dans le conteneur Docker), une page de configuration est accessible sur `/config`. Elle te permet de modifier tous les credentials et paramètres directement depuis le navigateur — sans SSH ni édition de fichier.
...
Les champs sensibles (clé API, tokens, secrets) ne sont jamais pré-remplis. Quand une valeur est déjà définie, le champ affiche `● ● ●  (already set)` comme placeholder. Soumettre le formulaire sans modifier un champ credential laisse la valeur stockée inchangée.

### Sauvegarde et redémarrage

Clique sur **Save configuration** pour écrire dans `config.toml`. Si tu modifies `serve_token`, le sidecar redémarre automatiquement (via supervisord dans Docker) et ton navigateur est redirigé vers `/login` une fois le serveur de retour.

### Bandeau de progression du fetch

Pendant qu'un fetch tourne en arrière-plan (déclenché depuis l'interface via le bouton ⬇), une bannière cyan apparaît en haut de la page bibliothèque avec la progression en direct (`idx/total · nom du jeu`). La page se recharge automatiquement à la fin du fetch.
