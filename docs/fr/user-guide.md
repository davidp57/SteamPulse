...
| `--backfill-alerts` | désactivé | Générer rétroactivement les alertes depuis l'historique des changements |
...
- Toutes les alertes générées par les règles configurées, triées par date décroissante
- Trois modes d’affichage : par règle, par jeu, combiné
- Suivi lu/non lu via `localStorage` — aucun serveur nécessaire
- Marquer une alerte comme lue individuellement, ou « Tout marquer comme lu »
- Filtre pour afficher toutes les alertes ou uniquement les non lues
- Clic sur un nom de jeu pour ouvrir sa fiche Steam
