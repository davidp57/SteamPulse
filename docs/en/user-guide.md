...
- **🔔 Alerts** — opens the alerts dashboard (configurable alert rules: price drops, updates, DLC, reviews…)
...
- All alerts generated by configured rules, sorted by newest first
- Three view modes: by rule, by game, combined
- Read/unread tracking via `localStorage` — no server needed
- Mark individual alerts as read, or use “Mark all as read”
- Filter by showing all alerts or only unread ones
- Click a game name to open its Steam store page
